...
def retrieve_image(filename, ra, dec):
    ...
    url = "http://legacysurvey.org/viewer/jpeg-cutout?ra={0}&dec={1}&zoom={2}&layer=des-dr1"
    #urllib.request.urlretrieve(url.format(ra, dec, 10), filename) #Retreaves and saves each image                                                  
    urllib.urlretrieve(url.format(ra, dec, 10), filename) #Retreaves and saves each image                                                  
